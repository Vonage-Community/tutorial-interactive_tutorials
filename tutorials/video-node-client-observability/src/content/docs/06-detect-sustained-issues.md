...
`findSustainedIssues()` tracks each session, connection, stream, and source independently and creates an alert only after three consecutive Warning or Critical records. A healthy record resets that sequence.
...
Save the file and reload the Video Quality Monitor.
