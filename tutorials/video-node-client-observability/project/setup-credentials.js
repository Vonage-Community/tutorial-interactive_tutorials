...
try {
  console.log("Vonage Video Client Observability setup");
  console.log("This stores credentials in the local .env file for this Codespace.");
  console.log("Find your account API key and API secret at https://dashboard.vonage.com/settings");
  console.log("Do not paste an Application ID, private key, JWT secret, or signature secret here.");
  console.log("Setup creates a new Vonage Video application in your dashboard.");
  console.log("");

  const apiKey = await prompt("Vonage account API key", process.env.VONAGE_API_KEY);
  const apiSecret = await prompt(
    "Vonage account API secret",
  ...
  const applicationName =
    `Client Observability Backend - ${process.env.CODESPACE_NAME || "local"}`;
  ...
  if (!application.id || !privateKey) {
    throw new Error(
      "Vonage did not return an application ID and private key."
    );
  }
