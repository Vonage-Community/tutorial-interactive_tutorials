...
We'll be creating the UI to recieve phone calls via the Vonage Voice API using the XR Blocks library's <a href="https://xrblocks.github.io/docs/api/classes/SpatialPanel/" target="_blank">SpatialPanel Class</a>.

The following code will be placed in the `VonageAudioCall.js` file.

First, let's initiate the constructor for our panel with some variables:
`// ⌄⌄⌄ Create constructor ⌄⌄⌄`
```js
constructor() {
  super();
...
setupVonageListeners() {
  ...
  this.client.on('callInvite', (callId, from, channelType) => {
    ...
    this.createCallPanel(maskedNumber);
  });
